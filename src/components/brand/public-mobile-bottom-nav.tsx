  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 block md:hidden pointer-events-none pb-[env(safe-area-inset-bottom)]">
      <nav 
        ...
        className="pointer-events-auto mx-4 mb-3 flex items-center justify-between rounded-full border border-white/20 bg-[#0c0a09]/90 px-4 py-2 text-white shadow-2xl backdrop-blur-xl transition-all"
      >
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex flex-col items-center justify-center py-1 px-2.5 rounded-full transition-all active:scale-90 ${
        ...
        <Link
          href="/reservation"
          className="group flex items-center gap-1.5 rounded-full bg-[#b7893b] px-4 py-2.5 text-xs font-bold text-black shadow-lg shadow-[#b7893b]/30 active:scale-95 transition-all"
        >
