...
import { bookingCalendarDate, bookingMonthDays, canSelectBookingDay, shiftBookingMonth } from "@/domain/models/public-booking-calendar";
...
export function BookingDatePicker({ value, minimum, onChange }: { value: string; minimum: string; onChange: (day: string) => void }) {
  ...
  function select(day: string) {
    if (!canSelectBookingDay(day, minimum)) { setError("Choisissez une date à partir de demain, hors vendredi (studio fermé)."); return; }
  ...
  return <section aria-label="Date de la séance" className="space-y-4 rounded-3xl border border-[#e5dacf] bg-white p-4 sm:p-6">
    ...
    <p className="text-sm text-[#61574b]">Vous pouvez choisir une date dans les mois suivants. Le studio est fermé le vendredi.</p>
