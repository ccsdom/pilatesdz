...
export function PublicSite() {
  return <div className={s.site}>
    ...
    <main id="contenu">
      ...
      <div className={s.facts}><div className={s.wrap}>{[[String(COURSE_MAX_CAPACITY), "places par créneau"], [String(COURSE_DURATION_MINUTES), "minutes pour vous"], ["Sur réservation", "selon les disponibilités"]].map(([value,label])=><div key={label}><strong>{value}</strong><span>{label}</span></div>)}</div></div>
