...
import styles from "./cash-overview.module.css";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { financeSurface as surface, financeMuted as muted, financeLink } from "./finance-layout";
...
export function CashOverview({ report, after }: { report: CashReport; after?: string }) {
  ...
  return <div className={styles.page}>
    <header className={styles.hero}>
      <div><p className={styles.eyebrow}>PILATES DZ · Encaissements</p><h1>Vos encaissements,<br /><em>en toute clarté.</em></h1><p className={styles.intro}>Retrouvez les espèces reçues, les corrections et le détail de chaque abonnement.</p></div>
      <div className={styles.actions}><Link href="/crm/forfaits" className={styles.primary}><Banknote size={18} aria-hidden="true" />Enregistrer un encaissement<ArrowUpRight size={16} aria-hidden="true" /></Link><Link href="/crm/encaissements/soldes" className={styles.secondary}>Vérifier les soldes<ArrowUpRight size={16} aria-hidden="true" /></Link></div>
    </header>
    <section className={styles.toolbar} aria-label="Période du rapport">
    ...
    <section className={styles.summary} aria-label={`Synthèse de ${monthLabel}`}><p className={`mb-3 text-xs capitalize ${muted}`}>{monthLabel} · Mois complet, toutes pages confondues</p><dl className="grid gap-4 md:grid-cols-3">
      <div className={styles.net}><dt className="flex items-center justify-between gap-3 text-xs">Espèces retenues<Banknote size={20} className="text-[#896127] dark:text-[#d8b97f]" /></dt><dd data-testid="cash-net" className="mt-5 break-words font-serif text-3xl tabular-nums">{formatCashAmount(summary.netMinor)}</dd><p className={`mt-3 text-xs ${muted}`}>{summary.count} encaissement(s) conservé(s), après corrections</p></div>
      <div className={styles.metric}><dt className={`flex items-center justify-between gap-3 text-xs ${muted}`}>Saisies annulées<RotateCcw size={18} /></dt><dd className="mt-5 break-words font-serif text-3xl tabular-nums">{formatCashAmount(summary.correctedMinor)}</dd><p className={`mt-3 text-xs ${muted}`}>{summary.correctedCount} correction(s), exclue(s) du total</p></div>
    ...
    <section className={styles.ledger + " " + surface} aria-labelledby="cash-ledger-title">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#b7893b]/15 p-5 sm:p-6"><div><p className={styles.sectionLabel}>Les mouvements du mois</p><h2 id="cash-ledger-title" className="font-serif text-2xl">Journal des encaissements</h2><p className={`mt-2 text-xs ${muted}`}>Espèces reçues selon la date de réception à Alger.</p></div><span className={`rounded-full bg-[#b7893b]/10 px-3 py-1.5 text-xs ${muted}`}>{entries.length} écriture(s) sur cette page</span></div>
      {entries.length === 0 ? <div className="px-6 py-16 text-center"><Receipt size={32} strokeWidth={1.2} className="mx-auto mb-4 text-[#b7893b]" /><h3 className="font-serif text-2xl">Aucun encaissement renseigné</h3><p className={`mt-2 text-sm ${muted}`}>Aucune écriture pour ce mois sur cette page.</p><Link className={`${financeLink} mt-5`} href="/crm/forfaits">Choisir une cliente<ArrowRight size={14} /></Link></div> : <ul className={styles.entries + " divide-y divide-[#b7893b]/10"}>{entries.map(entry => <li key={`${entry.clientId}/${entry.subscriptionId}/${entry.id}`} className="flex flex-wrap items-center gap-4 p-5 transition hover:bg-[#b7893b]/[0.03] sm:px-6">
