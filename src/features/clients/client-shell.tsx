...
import { UsersRound, LayoutDashboard, KeyRound } from "lucide-react";
import { BrandLockup } from "@/components/brand/brand-lockup";
import { LogoutButton } from "@/features/auth/logout-button";

export function ClientShell({ centerId, children }: { centerId: string; children: React.ReactNode }) {
  return <div className="min-h-screen bg-[#f3eee5] text-[#1c1b19] lg:flex">
    <aside className="bg-[#080808] p-5 text-white lg:w-64 lg:shrink-0"><div className="border-b border-white/10 pb-5"><BrandLockup light compact /></div>
      <nav aria-label="Navigation CRM" className="mt-5 flex flex-wrap gap-2 lg:flex-col">
        ...
        <Link href="/crm/clientes" aria-current="page" className="flex items-center gap-3 rounded-xl bg-[#b7893b] px-3 py-3 text-sm text-black"><UsersRound size={18} />Clientes</Link>
