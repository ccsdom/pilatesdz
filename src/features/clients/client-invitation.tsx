...
export function ClientInvitation({ details }: { details: ClientDetails }) {
  ...
  return <section className="space-y-4 rounded-2xl border border-[#ded4c3] bg-[#fffdf9] p-5 sm:p-7"><h2 className="font-serif text-2xl">Espace cliente</h2><p className="text-sm">{status}</p><p className="text-sm leading-6 text-muted-foreground">{cloud ? "L’e-mail Firebase permet à la cliente de choisir son mot de passe. Il est envoyé à l’adresse enregistrée dans cette fiche." : "Démonstration locale : aucun e-mail réel n’est envoyé. L’invitation utilise les coordonnées enregistrées dans cette fiche."}</p>
    {details.access !== "disabled" && details.profile.status === "active" && <Button type="button" variant="outline" onClick={invite} disabled={busy}>{busy ? "Préparation…" : cloud ? "Envoyer l’e-mail d’accès" : details.access === "active" ? "Préparer un nouveau lien" : "Inviter à l’espace cliente"}</Button>}
