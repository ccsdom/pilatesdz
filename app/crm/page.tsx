...
export default async function CrmPage() {
  ...
  return <><div className="flex flex-wrap items-center justify-between gap-3 bg-[#080808] px-5 py-4 text-sm text-[#d5ae65]"><span>Activité du centre · Heure d’Alger</span><div className="flex flex-wrap items-center gap-4"><Link href="/crm/planning" className="inline-flex items-center gap-1.5 underline hover:text-white transition-colors"><CalendarDays className="h-4 w-4" />Planning</Link><Link href="/crm/clientes" className="inline-flex items-center gap-1.5 underline hover:text-white transition-colors"><UsersRound className="h-4 w-4" />Clientes</Link><Link href="/crm/forfaits" className="inline-flex items-center gap-1.5 underline hover:text-white transition-colors"><WalletCards className="h-4 w-4" />Forfaits</Link><Link href="/crm/encaissements" className="inline-flex items-center gap-1.5 underline hover:text-white transition-colors"><CreditCard className="h-4 w-4" />Encaissements</Link><Link href="/crm/encaissements/soldes" className="inline-flex items-center gap-1.5 underline hover:text-white transition-colors"><Receipt className="h-4 w-4" />Soldes à vérifier</Link><Link href="/crm/acces" className="inline-flex items-center gap-1.5 underline hover:text-white transition-colors"><KeyRound className="h-4 w-4" />Gestion des accès</Link></div><LogoutButton /></div><CrmPreview attendance={<><AttendanceSummary access={result.access} at={now} /><section className="rounded-2xl border border-[#ded4c3] bg-[#fffdf9] p-5"><div className="flex items-center gap-2 mb-1"><WalletCards className="h-5 w-5 text-[#8d6729]" /><h2 className="font-serif text-2xl">Suivi des forfaits</h2></div><p className="my-3 text-sm">Repérez les échéances des sept prochains jours et les crédits faibles.</p><Link href="/crm/forfaits/suivi" className="inline-flex items-center gap-1.5 text-sm font-medium text-[#8d6729] underline"><WalletCards className="h-4 w-4" />Consulter les clientes à suivre</Link></section></>} summary={summary} clients={clients} planning={planning} dayLabel={dayLabel} /></>;
}
