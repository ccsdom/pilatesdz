...
export default async function Page({ searchParams }: { searchParams: Promise<{ after?: string }> }) {
  ...
  return <ClientShell centerId={result.access.centerId} active="packages">
    <header className="space-y-3"><Link href="/crm/forfaits" className="text-sm underline">Retour aux forfaits</Link><div className="flex items-center gap-3"><WalletCards className="h-8 w-8 text-[#b7893b]" /><h1 className="font-serif text-4xl">Forfaits à suivre</h1></div><p>Échéance dans les sept jours ou un crédit utilisable ou moins.</p><p className="text-sm text-muted-foreground">Fiches actives uniquement. Les crédits futurs sont séparés ; une fin de période mensuelle ne vaut pas fin d’abonnement trimestriel. Aucun message n’est envoyé automatiquement.</p></header>
