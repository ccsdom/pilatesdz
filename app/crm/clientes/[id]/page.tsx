...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <ClientShell centerId={result.access.centerId}><header className="space-y-3"><Link href="/crm/clientes" className="text-sm underline">Toutes les clientes</Link><h1 className="break-words font-serif text-4xl sm:text-5xl">{details.profile.name}</h1><p className="text-sm text-muted-foreground">Fiche du centre · Coordonnées et accès</p></header><Link href={`/crm/clientes/${details.profile.id}/forfaits`} className="inline-block rounded-xl bg-[#b7893b] px-5 py-3 text-sm">Forfaits et crédits</Link><Link href={`/crm/clientes/${details.profile.id}/historique`} className="inline-block rounded-xl border px-5 py-3 text-sm">Historique et assiduité</Link><ClientForm key={details.profile.id} profile={details.profile} /><ClientInvitation details={details} /></ClientShell>;
}
