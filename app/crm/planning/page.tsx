...
export default async function Page({ searchParams }: { searchParams: Promise<{ day?: string; after?: string }> }) {
  ...
  return <ClientShell centerId={result.access.centerId} active="planning"><header className="flex flex-wrap items-end justify-between gap-5"><div><p className="mb-2 text-sm text-[#957035]">Les séances du centre</p><h1 className="font-serif text-5xl">Planning</h1></div><Link href="/crm/planning/nouvelle" className="rounded-xl bg-[#111] px-5 py-3 text-sm font-medium text-white">Créer une séance</Link></header><PlanningCalendar page={page} admin now={now} /></ClientShell>;
}
