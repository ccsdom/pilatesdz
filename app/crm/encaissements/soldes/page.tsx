...
export default async function Page({ searchParams }: { searchParams: Promise<{ after?: string }> }) {
  ...
  return <ClientShell centerId={result.access.centerId} active="cash">
    <header className="space-y-3"><Link href="/crm/encaissements" className="text-sm underline">Retour aux encaissements</Link><h1 className="font-serif text-4xl">Soldes à vérifier</h1><p>Abonnements dont le montant n’est pas entièrement couvert par les encaissements saisis.</p><p className="text-sm text-muted-foreground">Une absence de saisie ne prouve pas un impayé. Vérifiez le journal avant tout échange avec la cliente. Les fiches inactives et les abonnements terminés restent inclus ; aucune relance n’est envoyée.</p></header>
