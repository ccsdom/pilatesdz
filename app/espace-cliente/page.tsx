...
export default async function Page({ searchParams }: { searchParams: Promise<{ day?: string; after?: string }> }) {
  ...
  return <main className="mx-auto min-h-screen max-w-6xl space-y-8 px-5 py-8 sm:px-8"><header className="flex flex-wrap items-center justify-between gap-5"><BrandLockup /><LogoutButton /></header><div className="space-y-3"><h1 className="font-serif text-4xl sm:text-5xl">Votre espace</h1><p>Choisissez votre séance et réservez votre place.</p><p className="text-sm text-muted-foreground">Un crédit est réservé pour chaque créneau, puis consommé après validation. Une annulation avant le créneau le libère. Le forfait doit être valable à la date choisie.</p></div><div className="flex flex-wrap items-center gap-3"><Link href="/espace-cliente/forfaits" className="inline-flex items-center gap-2 rounded-xl bg-[#b7893b] px-5 py-3 text-sm font-medium text-black"><WalletCards className="h-4 w-4" />Mes forfaits et crédits</Link><Link href="/espace-cliente/historique" className="inline-flex items-center gap-2 rounded-xl border px-5 py-3 text-sm font-medium"><History className="h-4 w-4" />Mes séances et mon assiduité</Link><Link href="/espace-cliente/mensurations" className="inline-flex items-center gap-2 rounded-xl border px-5 py-3 text-sm font-medium">Mes mensurations</Link></div><PlanningCalendar page={page} admin={false} now={now} /><Link href="/" className="block text-sm underline">Retour au studio</Link></main>;
}
