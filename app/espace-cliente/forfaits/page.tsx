...
export default async function Page({ searchParams }: { searchParams: Promise<{ after?: string; subscriptionAfter?: string }> }) {
  ...
  return <main className="mx-auto min-h-screen max-w-6xl space-y-8 px-5 py-8"><header className="flex flex-wrap items-center justify-between gap-5"><BrandLockup /><LogoutButton /></header><Link href="/espace-cliente" className="block text-sm underline">Retour au planning</Link><div className="flex items-center gap-3"><WalletCards className="h-8 w-8 text-[#b7893b]" /><h1 className="font-serif text-4xl">Mes forfaits et crédits</h1></div><p>Un crédit est consommé à la réservation et restitué en cas d’annulation avant le cours. La date d’expiration du forfait reste inchangée.</p><SubscriptionList page={subscriptions} now={getRequestTime()} after={subscriptionAfter} packageAfter={after} basePath="/espace-cliente/forfaits" /><PackageList subscriptionAfter={subscriptionAfter} page={page} now={getRequestTime()} after={after} basePath="/espace-cliente/forfaits" /></main>;
}
