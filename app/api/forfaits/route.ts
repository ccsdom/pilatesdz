...
const input = z.object({ requestId: z.string().uuid(), clientId: clientIdSchema, package: packageInputSchema }).strict();
...
export async function GET(request: NextRequest) {
  try {
    ...
    return json(await getPackageService().list(actor, request.nextUrl.searchParams.get("clientId") ?? undefined, request.nextUrl.searchParams.get("after") ?? undefined));
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!parsed.success) return json({ error: "Vérifiez le forfait, ses crédits et sa validité." }, 400);
    const data = parsed.data;
    return json({ package: await getPackageService().assign(actor, data.clientId, data.requestId, data.package) }, 201);
  } catch (error) { return failure(error); }
}
